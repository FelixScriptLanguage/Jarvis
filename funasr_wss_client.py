...
print(args)
pyttsx3.init()
requests.get('http://127.0.0.1:5970/pid?pid='+str(os.getpid()))
...
pyttsx3.speak('Jarvis has started')
...
async def message(id):
    ...
    try:
       while True:
            ...
            try:
                content = requests.get('http://127.0.0.1:5970/del').text
                if content == 'del':
                    text_print = ''
                    text_print_2pass_offline = ''
                    text_print_2pass_online = ''
                    #text_print_2pass_offline = text_print_2pass_offline[-1]
                    print('yes!!!')
            except:
                pass
            if 'mode' not in meg:
            ...
            print(text_print)
            try:
                print(eval(requests.get('http://127.0.0.1:5970/canSpeak').text))
                if eval(requests.get('http://127.0.0.1:5970/canSpeak').text):
                    requests.get('http://127.0.0.1:5970/content', params={'content': text_print})
            except:
                pass
...
if __name__ == '__main__':
    ...
    if args.audio_in is None:
        ...
        print('end')
'''Namespace(host='127.0.0.1', port=10095, chunk_size=[5, 10, 5], encoder_chunk_look_back=4, decoder_chunk_look_back=0, chunk_interval=10, hotword='', audio_in=None, audio_fs=16000, send_without_sleep=True, thread_num=1, words_max_print=10000, output_dir=None, ssl=1, use_itn=1, mode='2pass')
Namespace(host='127.0.0.1', port=10095, chunk_size=[5, 10, 5], encoder_chunk_look_back=4, decoder_chunk_look_back=0, chunk_interval=10, hotword='', audio_in=None, audio_fs=16000, send_without_sleep=True, thread_num=1, words_max_print=10000, output_dir=None, ssl=1, use_itn=1, mode='2pass')
